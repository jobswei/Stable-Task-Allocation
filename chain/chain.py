...
def count_unstable(match, tasks, workers):
    ...
    for t in tqdm.tqdm(range(len(tasks))):
        for w in range(len(workers)):
            if dist(tasks[t], workers[match[t].id]) > dist(tasks[t], workers[w]) and tasks[t].price > tasks[w2t[w]].price:
                count += 1
                break
    return count
...
def main(number):
    ...
    #退火前计算一下不稳定个数
    cnt = count_unstable(match_result_1, tasks, workers)
    ...
    #快速链式算法
    #match_result_3, dis_sum_3 = fast_chain(number, deepcopy(workers), deepcopy(tasks))
    ...
    return dis_sum_1, dis_sum_4, time_cost_1, time_cost_4, match_result_1, match_result_4, tasks, workers
...
def statistic():
    ...
    for n in nums:
        basic_dist, stable_chain_dist, basic_time, stable_chain_time, basic_match, stable_chain_match, tasks, workers = main(n)
        ...
        stable_count.append(n - count_unstable(stable_chain_match, tasks, workers))
    import pandas as pd
    ...
    plt.plot(nums, stable_chain_dists, marker='^')
...
def find_threshold(number, step=1):
    import math
    ...
    def score(threshold):
        ...
        print('score:{}, threshold:{}, unsable:{}, time(ms):{}'.format(s, threshold, unstable, time_cost.microseconds))
        return -s
    #from scipy.optimize import minimize
    #return minimize(score, x0=100)
    return {threshold: -score(threshold) for threshold in range(0, 200, step)}
...
if __name__ == '__main__':
    ...
    #statistic()
    #find_win_set(number)
    r = find_threshold(number, 1)
    import pandas as pd
    df = pd.DataFrame.from_dict(r, orient='index', columns=['score'])
    df.to_parquet('threshold1.parq')