...
def main(number):
    ...
    #退火前计算一下不稳定个数
    cnt = count_unstable(match_result_1, tasks_c, workers_c)
    print("退火前不稳定个数是：" + str(cnt))

    #基于baseline出来的数据进行退火
    anneal(number, match_result_1, workers, tasks_c, dis_sum_1)
    #退火后再计算下不稳定个数
    cnt = count_unstable(match_result_1, tasks_c, workers_c)
    ...
    #稳定链式算法
    #match_result_4, dis_sum_4 = stable_chain(number, workers_sc, tasks_sc)
    match_result_4, dis_sum_4, time_cost_4 = stable_chain(number, workers_sc, tasks_sc)
